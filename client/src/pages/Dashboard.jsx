 ...
 * FIXES IN THIS FILE:
 *
 *   Bug F — MostBorrowedBooks, AttendanceCount, TotalFinesCollected, OverdueBooks
 *     all call .reduce() on `data` directly at the top of the component body,
 *     before checking `loading` or `error`. If the parent renders before data
 *     arrives, `data` is whatever the useDashboard default is ([] is safe, but
 *     undefined or null causes "Cannot read properties of undefined (reading 'reduce')").
 *     Fix: all four components now default `data` to [] via destructuring, and
 *     each .reduce() is guarded so it never fires on a non-array.
 *
 *   No other logic was changed — all chart rendering, filter bar, and KPI card
...
import { useWebSocket } from "../hooks/useWebsocket"; // lowercase 's' — matches filename on disk
...
// FIX F: default data=[] so .reduce() never fires on undefined/null
const MostBorrowedBooks = React.memo(function MostBorrowedBooks({ data = [], loading, error }) {
  const safeData   = Array.isArray(data) ? data : [];
  const total      = safeData.reduce((s, r) => s + (r.borrows ?? 0), 0);
  const maxBorrows = safeData.length > 0 ? safeData[0].borrows : 1;
    ...
    >
      <div className="px-4 py-3 flex items-center justify-between gap-2 flex-shrink-0"
        ...
        <span className="flex items-center gap-1 text-[10px] font-bold px-2 py-0.5 rounded-full flex-shrink-0"
          ...
          <span className="inline-block w-1.5 h-1.5 rounded-full" style={{ background: "#EEA23A" }} />
          {loading ? "…" : `Top ${safeData.length}`}
        </span>
      </div>

      <div className="grid grid-cols-2 gap-2 px-4 py-2.5 flex-shrink-0"
          ...
          <table className="w-full" style={{ borderCollapse: "collapse" }}>
            <thead>
              <tr style={{ background: "var(--bg-subtle)" }}>
                <th className="px-4 py-2 text-left text-[9px] font-bold uppercase tracking-widest"
                  style={{ color: "var(--text-muted)", borderBottom: "1px solid var(--border-light)", width: 36 }}>
                  Rank
                </th>
                <th className="px-2 py-2 text-left text-[9px] font-bold uppercase tracking-widest"
                  style={{ color: "var(--text-muted)", borderBottom: "1px solid var(--border-light)" }}>
                  Title / Author
                </th>
                <th className="px-4 py-2 text-right text-[9px] font-bold uppercase tracking-widest"
            ...
            <tbody>
              {safeData.map((book, idx) => {
                const isTop3    = idx < 3;
                const medal     = MEDAL[idx];
                const rankColor = RANK_COLORS[idx % RANK_COLORS.length];
                const barPct    = maxBorrows > 0 ? ((book.borrows ?? 0) / maxBorrows) * 100 : 0;
                const isEven    = idx % 2 === 0;

                return (
                  <tr key={book.id ?? idx}
                    style={{ background: isEven ? "var(--bg-surface)" : "var(--bg-subtle)", transition: "background 0.12s" }}
                    ...
                    <td className="px-2 py-2.5"
                      ...
                      <p className="text-[11px] font-semibold leading-tight truncate"
                        style={{ color: "var(--text-primary)", maxWidth: 160 }} title={book.title}>
                        {book.title}
                      </p>
                      {book.author && (
                        <p className="text-[10px] mt-0.5 truncate"
                          style={{ color: "var(--text-muted)", maxWidth: 160 }} title={book.author}>
                          {book.author}
                        </p>
                      )}
                      <div className="mt-1.5 rounded-full overflow-hidden" style={{ height: 3, background: "var(--bg-subtle)" }}>
                        <div style={{ width: `${barPct}%`, height: "100%", background: rankColor, borderRadius: 9999, transition: "width 0.5s ease" }} />
                      </div>
                    </td>

                    <td className="px-4 py-2.5 text-right"
                      style={{ borderBottom: "1px solid var(--border-light)", verticalAlign: "middle" }}>
                      <span className="inline-block px-2 py-0.5 rounded-md text-[11px] font-extrabold tabular-nums"
...
// FIX F: default data=[] — AttendanceCount called data.reduce() before
// the loading check, crashing when data was undefined
const AttendanceCount = React.memo(function AttendanceCount({ data = [], loading, error, semester, month }) {
  const safeData = Array.isArray(data) ? data : [];
  const total    = safeData.reduce((s, r) => s + (r.visits ?? 0), 0);
  const peak     = safeData.reduce((mx, r) => (r.visits ?? 0) > mx.visits ? r : mx, { visits: 0, x: "—" });

  return (
    <Card title="Library Attendance" icon={Users} iconColor="#32667F" iconBg="rgba(50,102,127,0.15)">
      <StatPills items={[
      ...
      {loading ? (
        ...
        <div className="py-3 pl-1 pr-3" style={{ height: 190 }}>
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={safeData} margin={{ top: 6, right: 12, left: 0, bottom: 0 }}>
              <defs>
...
const TotalFinesCollected = React.memo(function TotalFinesCollected({ data = [], loading, error }) {
  const safeData       = Array.isArray(data) ? data : [];
  const totCollected   = safeData.reduce((s, r) => s + (r.collected   ?? 0), 0);
  const totUncollected = safeData.reduce((s, r) => s + (r.uncollected ?? 0), 0);

  return (
    <Card title="Fines Overview" icon={AlertTriangle} iconColor="#EA8B33" iconBg="rgba(234,139,51,0.15)">
      <StatPills items={[
      ...
      {loading ? (
        ...
        <div className="py-3 pl-1 pr-3" style={{ height: 190 }}>
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={safeData} barCategoryGap="35%" margin={{ top: 6, right: 12, left: 0, bottom: 0 }}>
              <CartesianGrid vertical={false} stroke="var(--border-light)" />
              <XAxis dataKey="x"
                tick={{ fontSize: 10, fill: "var(--text-secondary)", fontWeight: 600 }}
                axisLine={false} tickLine={false} interval={0} angle={-35} textAnchor="end" height={50} />
...
const OverdueBooks = React.memo(function OverdueBooks({ data = [], loading, error }) {
  const safeData = Array.isArray(data) ? data : [];
  const totCrit  = safeData.reduce((s, r) => s + (r.critical ?? 0), 0);
  const totWarn  = safeData.reduce((s, r) => s + (r.warning  ?? 0), 0);
  const totMin   = safeData.reduce((s, r) => s + (r.minor    ?? 0), 0);

  return (
    <Card title="Overdue Books" icon={AlertTriangle} iconColor="#dc2626" iconBg="rgba(220,38,38,0.12)">
      <StatPills items={[
        { value: totCrit, label: "Critical (15+ days)", color: "#dc2626", bg: "rgba(220,38,38,0.09)" },
        { value: totWarn, label: "Warning (8-14 days)", color: "#EA8B33", bg: "rgba(234,139,51,0.09)" },
        { value: totMin,  label: "Minor (1-7 days)",   color: "#EEA23A", bg: "rgba(238,162,58,0.09)" },
      ]} />
      {loading ? (
        <ChartSkeleton />
      ) : error ? (
        ...
        <div className="flex items-center justify-center py-8">
          <span className="text-[12px]" style={{ color: "var(--text-muted)" }}>No overdue books</span>
        </div>
      ) : (
        <div className="py-3 pl-1 pr-3" style={{ height: 190 }}>
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={safeData} barCategoryGap="35%" margin={{ top: 6, right: 12, left: 0, bottom: 0 }}>
              ...
              <Bar dataKey="warning"  name="Warning"  fill="#EA8B33" radius={[3, 3, 0, 0]} stackId="a" />
              <Bar dataKey="minor"    name="Minor"    fill="#EEA23A" radius={[3, 3, 0, 0]} stackId="a" />
            </BarChart>
          </ResponsiveContainer>
        </div>
      )}
...
// Skeleton rows for the holdings table while loading
      ...
      {Array.from({ length: 6 }).map((_, i) => (
        <tr key={i} className="animate-pulse">
          {Array.from({ length: 7 }).map((__, j) => (
            <td key={j} style={{ borderBottom: "1px solid var(--border-light)", padding: "14px 16px" }}>
              <div className="h-3 rounded-full" style={{ background: "var(--border)", width: j === 1 ? "60%" : "40%" }} />
            </td>
          ))}
        </tr>
      ))}
...
// FIX F: default data=[]
const HoldingsBreakdown = React.memo(function HoldingsBreakdown({ data = [], loading, error }) {
  const safeData    = Array.isArray(data) ? data : [];
  const totalNemco  = safeData.reduce((s, r) => s + (r.nemco  ?? 0), 0);
  const totalLexora = safeData.reduce((s, r) => s + (r.lexora ?? 0), 0);
      ...
      <div className="px-5 py-3.5 flex items-center justify-between flex-wrap gap-3"
        ...
        <div className="flex items-center gap-2.5">
          <div className="w-6 h-6 rounded-lg flex items-center justify-center"
            style={{ background: "rgba(19,47,69,0.1)" }}>
            <Library size={13} style={{ color: NEMCO_COLOR }} />
          </div>
          <h2 className="text-[11px] font-bold uppercase tracking-wider" style={{ color: "var(--text-secondary)" }}>
            Holdings Breakdown by Program / Category
          </h2>
        </div>

        <div className="flex items-center gap-3 flex-wrap">
          <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl"
            style={{ background: "rgba(19,47,69,0.05)", border: "1px solid rgba(19,47,69,0.1)" }}>
      ...
      {error ? (
        ...
        <div className="overflow-x-auto">
          <table className="w-full" style={{ borderCollapse: "collapse" }}>
            <thead>
              <tr style={{ background: "var(--bg-subtle)" }}>
                <th className="px-5 py-3 text-left text-[10px] font-bold uppercase tracking-widest"
                  style={{ color: "var(--text-muted)", width: 42, borderBottom: "2px solid var(--border)" }}>#</th>
                <th className="px-4 py-3 text-left text-[10px] font-bold uppercase tracking-widest"
                  style={{ color: "var(--text-muted)", borderBottom: "2px solid var(--border)" }}>Program / Category</th>
                <th colSpan={2} className="py-0 text-center text-[10px] font-bold uppercase tracking-widest"
                ...
                <th colSpan={2} className="py-0 text-center text-[10px] font-bold uppercase tracking-widest"
                  ...
                  <div className="flex items-center justify-center gap-1.5 px-4 py-3"
                    ...
                    <Library size={11} /> LEXORA
                  </div>
                </th>
                <th className="px-5 py-3 text-right text-[10px] font-bold uppercase tracking-widest"
                  style={{ color: "var(--text-muted)", borderBottom: "2px solid var(--border)", borderLeft: "1px solid var(--border-light)" }}>Total</th>
              </tr>
              <tr style={{ background: "var(--bg-subtle)" }}>
                <th style={{ borderBottom: "1px solid var(--border-light)" }} />
                <th style={{ borderBottom: "1px solid var(--border-light)" }} />
                <th className="px-4 py-2 text-[9px] font-bold uppercase tracking-widest text-center"
                  style={{ color: NEMCO_COLOR, opacity: 0.75, borderBottom: "1px solid var(--border-light)", borderLeft: `2px solid ${NEMCO_COLOR}22` }}>Count</th>
                <th className="px-4 py-2 text-[9px] font-bold uppercase tracking-widest text-center"
                  style={{ color: NEMCO_COLOR, opacity: 0.75, borderBottom: "1px solid var(--border-light)" }}>Share</th>
                <th className="px-4 py-2 text-[9px] font-bold uppercase tracking-widest text-center"
                  style={{ color: LEXORA_COLOR, opacity: 0.75, borderBottom: "1px solid var(--border-light)", borderLeft: `2px solid ${LEXORA_COLOR}22` }}>Count</th>
                <th className="px-4 py-2 text-[9px] font-bold uppercase tracking-widest text-center"
                  style={{ color: LEXORA_COLOR, opacity: 0.75, borderBottom: "1px solid var(--border-light)" }}>Share</th>
            ...
            <tbody>
              {loading ? (
                ...
                safeData.map((row, idx) => {
                  const rowTotal    = (row.nemco ?? 0) + (row.lexora ?? 0);
                  const nemcoShare  = rowTotal > 0 ? (((row.nemco ?? 0) / rowTotal) * 100).toFixed(1) : "0.0";
                  const lexoraShare = rowTotal > 0 ? (((row.lexora ?? 0) / rowTotal) * 100).toFixed(1) : "0.0";
                  const isEven      = idx % 2 === 0;
                  ...
                  return (
                    <tr key={row.category}
                      ...
                      onMouseEnter={e => (e.currentTarget.style.background = "var(--bg-hover)")}
                      onMouseLeave={e => (e.currentTarget.style.background = isEven ? "var(--bg-surface)" : "var(--bg-subtle)")}>
                      <td className="px-5 py-3.5 text-[11px] font-semibold tabular-nums"
                        style={{ color: "var(--text-muted)", borderBottom: "1px solid var(--border-light)" }}>{idx + 1}</td>
                      <td className="px-4 py-3.5" style={{ borderBottom: "1px solid var(--border-light)" }}>
                        <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-[11px] font-bold uppercase tracking-wide"
                          style={{ background: "rgba(50,102,127,0.08)", color: "var(--text-primary)", border: "1px solid rgba(50,102,127,0.14)" }}>
                          {row.category}
                        </span>
                      </td>
                      <td className="px-4 py-3.5 text-center"
                        style={{ borderBottom: "1px solid var(--border-light)", borderLeft: `2px solid ${NEMCO_COLOR}22` }}>
                        <span className="inline-block px-3 py-1 rounded-lg text-[12px] font-extrabold tabular-nums"
                          style={{ background: (row.nemco ?? 0) > 0 ? "rgba(19,47,69,0.07)" : "transparent", color: (row.nemco ?? 0) > 0 ? NEMCO_COLOR : "var(--text-muted)" }}>
                          {(row.nemco ?? 0).toLocaleString()}
                        </span>
                      </td>
                      <td className="px-4 py-3.5 text-center" style={{ borderBottom: "1px solid var(--border-light)" }}>
                        <span className="text-[11px] font-semibold tabular-nums"
                          style={{ color: (row.nemco ?? 0) > 0 ? NEMCO_COLOR : "var(--text-muted)", opacity: (row.nemco ?? 0) > 0 ? 0.75 : 0.4 }}>
                          {nemcoShare}%
                        </span>
                      </td>
                      <td className="px-4 py-3.5 text-center"
                        style={{ borderBottom: "1px solid var(--border-light)", borderLeft: `2px solid ${LEXORA_COLOR}22` }}>
                        <span className="inline-block px-3 py-1 rounded-lg text-[12px] font-extrabold tabular-nums"
                          style={{ background: (row.lexora ?? 0) > 0 ? "rgba(50,102,127,0.08)" : "transparent", color: (row.lexora ?? 0) > 0 ? LEXORA_COLOR : "var(--text-muted)" }}>
                          {(row.lexora ?? 0).toLocaleString()}
                        </span>
                      </td>
                      <td className="px-4 py-3.5 text-center" style={{ borderBottom: "1px solid var(--border-light)" }}>
                        <span className="text-[11px] font-semibold tabular-nums"
                          style={{ color: (row.lexora ?? 0) > 0 ? LEXORA_COLOR : "var(--text-muted)", opacity: (row.lexora ?? 0) > 0 ? 0.75 : 0.4 }}>
                          {lexoraShare}%
                        </span>
                      </td>
                      <td className="px-5 py-3.5 text-right"
                        style={{ borderBottom: "1px solid var(--border-light)", borderLeft: "1px solid var(--border-light)" }}>
                        <span className="text-[12px] font-extrabold tabular-nums" style={{ color: "var(--text-primary)" }}>
            ...
            {!loading && safeData.length > 0 && (
              <tfoot>
                <tr style={{ background: "var(--bg-subtle)" }}>
                  ...
                  <td className="px-4 py-3.5 text-[11px] font-bold uppercase tracking-wider"
                    style={{ color: "var(--text-secondary)", borderTop: "2px solid var(--border)" }}>Grand Total</td>
                  <td className="px-4 py-3.5 text-center"
                  ...
                  <td className="px-4 py-3.5 text-center" style={{ borderTop: "2px solid var(--border)" }}>
...
export default function Dashboard() {
  ...
  const stats = useMemo(() => {
    ...
    return [
      { label: "NEMCO BOOKS",  value: Number(nemcoTotal).toLocaleString(),      change: "Live", accent: "#132F45", percentage: nemcoTotal ? 100 : undefined, onClick: () => navigate("/dashboard/books") },
      { label: "LEXORA BOOKS", value: Number(lexoraTotal).toLocaleString(),     change: "Live", accent: "#32667F", percentage: lexoraTotal ? 100 : undefined, onClick: () => navigate("/dashboard/lexora-books") },
      { label: "Out of Stock", value: Number(nemcoOutOfStock).toLocaleString(), change: "Live", accent: "#dc2626", percentage: pctOf(nemcoOutOfStock, nemcoTotal), onClick: () => navigate("/dashboard/books?status=OutOfStock") },
      { label: "Returned",     value: Number(returned).toLocaleString(),        change: "Live", accent: "#32667F", percentage: pctOf(returned, nemcoTotal),        onClick: () => navigate("/dashboard/borrowed?status=Returned") },
    ];
  ...
  return (
    <main className="flex flex-col gap-4 lg:gap-5" aria-label="Library Analytics Dashboard">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 className="flex items-center gap-2.5 text-[22px] font-bold" style={{ color: "var(--text-primary)" }}>
      ...
      <section className="grid grid-cols-2 lg:grid-cols-4 gap-3 lg:gap-4">
        {stats.map(stat => (
          <StatsCard key={stat.label} label={stat.label} value={stat.value}
            change={stat.change} accent={stat.accent} percentage={stat.percentage} onClick={stat.onClick} />
        ))}
      </section>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-4">
        <div className="lg:col-start-1 lg:col-span-2 lg:row-start-1 lg:row-span-2">
          <MostBorrowedBooks data={mostBorrowed} loading={loading.charts} error={errors.mostBorrowed} />
        </div>
        <div className="lg:col-start-3 lg:col-span-3 lg:row-start-1">
          <AttendanceCount data={attendance} loading={loading.charts} error={errors.attendance} semester={semester} month={month} />
        </div>
        <div className="lg:col-start-3 lg:col-span-3 lg:row-start-2">
          <TotalFinesCollected data={fines} loading={loading.charts} error={errors.fines} semester={semester} month={month} />
        </div>
        <div className="lg:col-start-1 lg:col-span-5 lg:row-start-3">
          <OverdueBooks data={overdue} loading={loading.charts} error={errors.overdue} />
        </div>
      </div>
