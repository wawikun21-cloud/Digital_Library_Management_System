...
export default function LexoraBooks() {
  ...
  useEffect(() => {
    setCurrentPage(1);
  }, [query, programFilter, resourceTypeFilter, sortBy, pageSize]);
  ...
  // ── Handlers ─────────────────────────────────────────────
  const openModal = useCallback((mode = "lexora") => {
    setDdOpen(false);
  ...
  function handleViewDetails(book) {
    ...
    setModalMode("view");
    setForm({ ...EMPTY_FORM, ...book, _mode: "lexora" });
    setModal(true);
  }
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      {/* ── Detail modal — Lexora view ── */}
      <LexoraBookDetailModal
        isOpen={modal && modalMode === "view"}
      ...
      {/* ── Add / edit modal ── */}
      <BookModal
        isOpen={modal && modalMode !== "view"} onClose={handleCloseModal}
...
function Pagination({ currentPage, totalPages, totalItems, pageSize, onPageChange, onPageSizeChange }) {
  ...
  return (
    <div
      className="flex flex-wrap items-center justify-between gap-3 px-1 pt-1"
      style={{ color: "var(--text-secondary)" }}
    >
...
function FilterBadge({ label, onClear }) {
  return (
    <span
      className="flex items-center gap-1 px-2.5 py-1 rounded-full text-[11.5px] font-semibold"
      style={{
        background: "rgba(238,162,58,0.12)",
        color: "var(--accent-amber)",
        border: "1px solid rgba(238,162,58,0.3)",
      }}
    >
      {label}
