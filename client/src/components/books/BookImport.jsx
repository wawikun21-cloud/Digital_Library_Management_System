...
/* ─── Column indices ────────────────────────────────────────
   Acc. No. | Date | Class | Title | Author |
...
/* ── Parser ─────────────────────────────────────────────────── */
function parseStandardExcel(buffer) {
  ...
  for (const row of rows.slice(2)) {
    ...
    if (title) {
      ...
      current = {
        ...
        shelf: null, place: null, edition: null, materialType: null,
        genre: null, isbn: null, issn: null, lccn: null,
        subtitle: null, extent: null, size: null, description: null, otherDetails: null,
      };
    } else if (acc && current) {
...
const BookImport = forwardRef(function BookImport({ onImportComplete, onStepChange }, ref) {
  ...
  const handleFile = useCallback((file) => {
    ...
    reader.onload = (e) => {
      try {
        const result = parseStandardExcel(new Uint8Array(e.target.result));
        if (result.length === 0) {
          setParseError("No book records found. Check that the file matches the expected format.");
          return;
        }
        setParsed(result);
        setPreview(result.slice(0, 8));
        setStepAndNotify(2, result);
      } catch (err) {
  ...
  const startImport = async () => {
    ...
    try {
      ...
      // If check fails, proceed — server handles duplicates
    } finally {
      setChecking(false);
    }
    await runImport();
  ...
  const runImport = async () => {
    ...
    // Split into chunks so we can update progress between each chunk
    for (let offset = 0; offset < total; offset += CHUNK_SIZE) {
      const chunk     = parsed.slice(offset, offset + CHUNK_SIZE);
      const chunkEnd  = Math.min(offset + CHUNK_SIZE, total);

      // Show the first title of this chunk while it's in-flight
      ...
      try {
        ...
        if (data.success) {
          ...
          // Whole chunk failed — mark each book as failed
  ...
  const reset = () => {
    setStepAndNotify(1);
    setFileName(""); setParsed([]); setPreview([]);
    setProgress({ current:0, total:0, title:"" });
    setResults(null); setParseError(""); setDupAlert(null); setChecking(false);
    if (fileRef.current) fileRef.current.value = "";
  };

  useImperativeHandle(ref, () => ({ step, parsed, startImport, reset, checking }), [step, parsed, startImport, reset, checking]);
    ...
    <div className="flex flex-col gap-4">
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4"
          ...
          <div className="flex flex-col gap-4 w-full max-w-[520px] rounded-2xl shadow-2xl overflow-hidden"
            ...
            <div className="flex items-center gap-3 px-5 pt-5">
              ...
              <div>
                <p className="text-[14px] font-black" style={{ color:"var(--text-primary)" }}>Duplicates Detected</p>
                <p className="text-[11px]" style={{ color:"var(--text-secondary)" }}>The following already exist in the database.</p>
              </div>
            </div>
            ...
            {dupAlert.duplicateTitles.length > 0 && (
              <div className="flex flex-col gap-1.5 px-5">
                ...
                <div className="rounded-xl border overflow-hidden max-h-[140px] overflow-y-auto"
                  ...
                  {dupAlert.duplicateTitles.map((d, i) => (
                    <div key={i} className="flex items-center gap-2 px-3 py-2 border-b text-[11px]"
                      ...
                      <span className="font-semibold truncate" style={{ color:"var(--text-primary)" }}>{d.title}</span>
                    </div>
                  ))}
                </div>
              </div>
            ...
            {dupAlert.duplicateAccessions.length > 0 && (
              <div className="flex flex-col gap-1.5 px-5">
                ...
                <div className="rounded-xl border overflow-hidden max-h-[120px] overflow-y-auto"
                  ...
                  {dupAlert.duplicateAccessions.map((d, i) => (
                    <div key={i} className="flex items-center gap-2 px-3 py-2 border-b text-[11px]"
                      ...
                      <span className="font-mono font-bold" style={{ color:"var(--accent-amber)" }}>{d.accession}</span>
            ...
            <div className="mx-5 flex items-start gap-2 px-3 py-2.5 rounded-xl text-[11px]"
              ...
              <span>Existing titles will have their new copies added. Duplicate accession numbers will be skipped automatically.</span>
            </div>
      ...
      {step === 1 && (
        <>
          <div className="flex items-start gap-3 p-3 rounded-xl"
            ...
            <div>
              <p className="text-[12px] font-semibold" style={{ color:"var(--text-primary)" }}>Bulk Import from Excel</p>
              <p className="text-[11px] mt-0.5" style={{ color:"var(--text-secondary)" }}>
                Expected columns:{" "}
                <span className="font-mono text-[10px]">
                  Acc. No. | Date | Class | Title | Author | Vol./Ed. | Year | Publisher | Pages | Coll.
                </span>
              </p>
              <p className="text-[11px] mt-0.5" style={{ color:"var(--text-secondary)" }}>
        ...
        <>
          ...
          <div className="flex items-start gap-2 px-3 py-2.5 rounded-xl text-[11px]"
            ...
            <span>
              ...
              Each copy accession is saved individually in the book_copies table.
            </span>
          </div>
        </>
      )}
      ...
      {step === 3 && (
        <div className="flex flex-col items-center gap-5 py-6">
          ...
          <div className="text-center">
            <p className="text-[14px] font-bold" style={{ color:"var(--text-primary)" }}>Importing books…</p>
          ...
          <div className="grid grid-cols-3 gap-3">
            ...
            <div className="flex flex-col items-center gap-1.5 p-4 rounded-xl"
              ...
              <p className="text-2xl font-black" style={{ color:"var(--accent-amber)" }}>
                {results.updated ?? results.existing ?? 0}
              </p>
              <p className="text-[10px] font-bold uppercase tracking-wider" style={{ color:"var(--accent-amber)" }}>Existing</p>
            </div>
            <div className="flex flex-col items-center gap-1.5 p-4 rounded-xl"
                 style={{
                   background: (results.errors ?? results.failedBooks ?? 0) > 0 ? "rgba(220,38,38,0.07)" : "var(--bg-subtle)",
                   border: `1.5px solid ${(results.errors ?? results.failedBooks ?? 0) > 0 ? "rgba(220,38,38,0.2)" : "var(--border-light)"}`,
                 }}>
              <XCircle size={26} style={{ color:(results.errors ?? results.failedBooks ?? 0) > 0 ? "#b91c1c" : "var(--text-muted)" }} />
              <p className="text-2xl font-black"
                 style={{ color:(results.errors ?? results.failedBooks ?? 0) > 0 ? "#b91c1c" : "var(--text-muted)" }}>
                {results.errors ?? results.failedBooks ?? 0}
              </p>
              <p className="text-[10px] font-bold uppercase tracking-wider"
                 style={{ color:(results.errors ?? results.failedBooks ?? 0) > 0 ? "#b91c1c" : "var(--text-muted)" }}>Failed</p>
            </div>
          </div>
