...
export default function BorrowedCard({ 
  ...
  const initials = transaction.bookTitle.split(" ").slice(0, 2).map(w => w[0]).join("").toUpperCase();
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col gap-2 p-2.5 flex-1">
        ...
        {/* Due Date */}
        <div className="flex items-center gap-1 text-[10px]" style={{ color: isOverdue(transaction.due) ? "#EA8B33" : "var(--text-secondary)" }}>
          <CalendarClock size={12} />
          <span className="font-medium">{formatDate(transaction.due)}</span>
        </div>
